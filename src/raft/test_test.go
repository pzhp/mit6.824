...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("disconnected leader: %d", leader1)
	cfg.checkOneLeader()
	...
	DPrintf("disconnected leader: %d", leader2)
	cfg.disconnect((leader2 + 1) % servers)
	DPrintf("disconnected leader: %d", (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	DPrintf("reconnected leader: %d", (leader2+1)%servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	DPrintf("reconnected leader: %d", leader2)
	cfg.checkOneLeader()
